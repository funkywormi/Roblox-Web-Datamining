...
const VpcPrologue = ({
  ...
  const [requireVpcModal, requireVpcModalService] = useModal({
    ...
    onAction: () => {
      sendEmailParentClickEvent(featureName, true, settingName, recourseParameters ?? undefined);
      if (onProceed) {
    ...
    size: 'sm',
    onHide: () => {
      sendVerifyCancelClickEvent(featureName, 'Vpc', settingName, recourseParameters ?? undefined, source);
      onHide();
    },
    onNeutral: () => {
      sendVerifyCancelClickEvent(featureName, 'Vpc', settingName, recourseParameters ?? undefined, source);
  ...
  useEffect(() => {
    ...
    sendInitialUpsellPageLoadEvent(featureName, 'Vpc', settingName, recourseParameters ?? undefined, source);
  }, []);
