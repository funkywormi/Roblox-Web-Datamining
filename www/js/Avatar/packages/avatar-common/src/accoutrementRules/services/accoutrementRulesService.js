...
function insertAssetMetaIntoAssetList(assetToAdd, assetList) {
  ...
  // The DNA layered accessories (hair, eyebrows, eyelashes) carry a layered clothing meta.order but
  // are excluded from layered clothing ordering, so they are absent from layeredClothingAssets.
  // Reordering around one of them would shift every real layered clothing item down a layer.
  if (!isLayeredClothing(assetToAdd.assetType?.id, true)) {
    return filteredAssets;
  }
  ...
  if (newAssetToAdd && assetToAdd.meta && !assetOrdersUnchanged) {
