...
import { TDetailEntry } from "@rbx/legacy-webapp-types/Roblox";
...
function buildOfferEligibleItems(
  ...
  selectedItemsList.forEach(item => {
    ...
    if (
      ...
      detail.timedOptions?.some((option: TTimedOption) => option.days === selectedTimedOption.days)
    ) {
...
function ItemPrice({
  ...
  if (item.timedOptions && item.timedOptions.length > 0 && itemDetails.timedOptions?.length) {
    const selectedTimedOption = item.timedOptions.find((opt: TTimedOption) => opt.selected);
    ...
    const matchingTimedOption = itemDetails.timedOptions.find(
      (opt: TTimedOption) => opt.days === selectedDays,
    );
  ...
  const isLimited = itemRestrictions.some(
    (r: string) => r === "Limited" || r === "LimitedUnique" || r === "Collectible",
  );
  if (isLimited) {
...
function ShoppingCartModalItem({
  ...
  const persistedSelectedOption =
    item.timedOptions?.find((option: TTimedOption) => option.selected) ?? item.timedOptions?.[0];
  const isSelectedDurationStillOffered =
    !!persistedSelectedOption &&
    (persistedSelectedOption.days === 0 ||
      !!itemDetails.timedOptions?.some(
        (option: TTimedOption) => option.days === persistedSelectedOption.days,
      ));
  ...
  const permanentTimedOption = offeredTimedOptions.find(
    (option: TTimedOption) => option.days === 0,
  );
  const rentalTimedOptions = offeredTimedOptions.filter((option: TTimedOption) => option.days > 0);
  ...
  const offerAdjustedTotalPrice = useMemo(() => {
    ...
    selectedItemsList.forEach(item => {
      ...
      const matchingTimedOption = detail.timedOptions?.find(
        (option: TTimedOption) => option.days === selectedTimedOption?.days,
      );
  ...
  const wasTimedOptionPurchased = useCallback(
    (itemDetail: TDetailEntry) => {
      ...
      return (
        ...
        !!itemDetail.timedOptions?.some((opt: TTimedOption) => opt.days === selectedOption.days)
      );
    },
    [selectedItemsList],
  );
    ...
    (
      ...
      results.forEach(result => {
        ...
        if (data?.reason === "Success") {
          ...
          if (data.itemData.assetId !== undefined) {
            itemDetail = itemDetails[data.itemData.assetId];
            ...
            dispatch(removeItemAction({ itemId: data.itemData.assetId, itemType: "Asset" })).catch(
              () => {
                console.error("Failed to remove item");
              },
            );
          } else if (data.itemData.bundleId !== undefined) {
            itemDetail = itemDetails[data.itemData.bundleId];
            const itemPrice = getTransactionItemPrice(itemDetail);
            totalTransactionValue += itemPrice;
            purchasedItems.push({
              itemType: itemDetail.itemType,
            ...
            dispatch(
              removeItemAction({ itemId: data.itemData.bundleId, itemType: "Bundle" }),
            ).catch(() => {
              console.error("Failed to remove item");
            });
          }
    ...
    selectedItemsList.forEach(item => {
      ...
      const itemStillOffersTimedOption =
        ...
        !!itemDetail?.timedOptions?.some(
          (opt: TTimedOption) => opt.days === selectedTimedOption.days,
        );
