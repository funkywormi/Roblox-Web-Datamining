...
import { isAuthenticated, isPremiumUser, userId } from "@rbx/core-scripts/meta/user";
import { EnvironmentUrls } from "@rbx/environment-urls";
import { TDiscountInformation } from "../../itemDetailsHydrationService/constants/itemDetailsHydrationConstants";
import {
  TCartState,
  TCartItem,
  TItemDetails,
...
function getCartStorageKey() {
  const currentUserId = isAuthenticated() && userId();
  return `Roblox.AvatarMarketplace.Cart:${currentUserId || ""}`;
}
...
export function fetchCartState(): TCartState {
  ...
  return {
    ...
    purchaseParams: {},
    itemDetails: {},
    ...(localStorageService.getLocalStorage(key) as Partial<TCartState>),
  } as TCartState;
}
...
export function getItemPrice(itemDetail: TItemDetails, timedOptions?: TTimedOption[]): number {
  ...
  if (timedOptions && timedOptions.length > 0 && itemDetail.timedOptions?.length) {
    ...
    const matchingTimedOption = itemDetail.timedOptions.find(
      (opt: TTimedOption) => opt.days === selectedOption?.days,
    );
...
export function splitItemsByCollectible(items: TCartItem[] = []): TCartItem[][] {
  const itemTypeCategorizer = (
    [legacyItems, collectibleItems]: [TCartItem[], TCartItem[]],
    item: TCartItem,
  ): [TCartItem[], TCartItem[]] =>
  ...
  return items.reduce<[TCartItem[], TCartItem[]]>(itemTypeCategorizer, [[], []]);
}
