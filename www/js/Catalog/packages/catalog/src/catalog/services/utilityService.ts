...
class UtilityService {
  static buildItemDetailsUrl(item: ItemWithDetails): string {
    ...
    switch (itemType) {
      case itemTypes.bundle:
        return getAbsoluteUrl(`/bundles/${id}/`);
      case itemTypes.asset:
      default:
        return getAbsoluteUrl(`/catalog/${id}/`);
    }
  }
  ...
  static formatQueries(queryParams: QueryParams, cursor?: string): Query {
  ...
  static translateToEnumStrings(
    ...
    if (category && categoryEnumLibrary?.[category]) {
      ...
      if (subcategoryEnums) {
        ...
        if (gears) {
          subcategoryString = subcategoryEnums[gears] ?? null;
        } else if (subcategory) {
          subcategoryString = subcategoryEnums[subcategory] ?? null;
        }
  ...
  static buildUserLink(item: Pick<ItemWithDetails, "creatorType" | "creatorTargetId">): string {
    ...
    switch (creatorType) {
      case userTypes.group:
        return getAbsoluteUrl(`/groups/${creatorTargetId}`);
      case userTypes.user:
      default:
        return getAbsoluteUrl(`/users/${creatorTargetId}/profile`);
    }
  }
  ...
  static buildErrorMessages = (errors: AxiosError[], userAgent: string): string => {
    let errorMessage = `userId-${userId()}`;
