...
function CatalogPage({
  ...
  const { getPagedItems, fetchNextPage, canLoadNextPage, searchResultDict, searchResultList } =
    useCatalogSearch({
      catalogQuery,
      searchOptions,
  ...
  const clearPrice = (keepKeyword: boolean) => {
  ...
  const onTopUpClear = () => {
    localStorageService.setLocalStorage(
      `Roblox.AvatarMarketplace.TopUpClearDate-${userId()}`,
      Date.now(),
    );
  ...
  const { mobileSearchOptionsContainerHeight, rbxHeaderHeight, headingContainerHeight } =
