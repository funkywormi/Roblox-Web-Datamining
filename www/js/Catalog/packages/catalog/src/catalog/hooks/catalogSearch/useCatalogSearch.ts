...
const useCatalogSearch = (params: CatalagSearchParams) => {
  ...
  const shouldUseMarketplaceWidgetsLandingPage =
    ...
    isAuthenticated() &&
    isUserInMarketplaceWidgetsLandingPageRollout(userId());

  const { startPaging, setStartPaging, nextPageCursor, setNextPageCursor, resetPagination } =
    usePagination();
  ...
  const buildElasticSearchDebugMap = useCallback(
    (
      ...
      const elasticSearchDebugInfoMap = new Map<string, SearchItemsResponseDataSourceInfo>();
  ...
  const buildSearchResultData = useCallback(
    (
      ...
      const elasticSearchDebugInfoMap: Map<string, SearchItemsResponseDataSourceInfo> =
      ...
      searchData?.forEach((originalItem: TItem) => {
        ...
        if (!searchResultList?.includes(key)) {
