...
function useMetadata() {
  ...
  const [isPremiumPriceOnItemTilesEnabled, setIsPremiumPriceOnItemTilesEnabled] =
    useState<boolean>();
  const [isPremiumIconOnItemTilesEnabled, setIsPremiumIconOnItemTilesEnabled] = useState<boolean>();
  const [autocompleteAvatarSearchNumToDisplay, setAutocompleteAvatarSearchNumToDisplay] =
    useState<number>();
  const [isCatalogAdsRowOnRecommendedPageEnabled, setIsCatalogAdsRowOnRecommendedPageEnabled] =
    useState<boolean>();
