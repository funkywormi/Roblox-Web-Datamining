...
import {
  getTraceIdFromUrl,
  STUDIO_TRACE_ID_PARAM,
} from "@rbx/authentication-common/utils/traceIdUtils";
import {
  getSignupErrorField,
  getSignupErrorState,
  SignupErrorOutcome,
 ...
 *
 * The `dismissed`, `unsupported`, and `error` outcomes forward their `reason`
 * as `passkeyFailureReason` when present. `dismissed` carries one because
 * WebAuthn collapses user-cancel, timeout, and several platform rejections into
 * a single `NotAllowedError`; surfacing the underlying cause keeps that
 * distinction visible in telemetry instead of flattening it to a bare
 * dismissal.
 */
