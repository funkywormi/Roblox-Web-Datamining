...
import { fido2Util } from "@rbx/core-scripts/legacy/core-roblox-utilities";
 ...
 * `dismissed:`, `unsupported:`) so `SPLIT(reason, ':')[1]` yields the failure
 * class. Keep the vocabulary closed — a browser-supplied string would be an
 * unbounded group key.
 */
export type PasskeyAttemptOutcome =
  ...
  | { kind: "unsupported"; reason: PasskeyUnsupportedReason }
...
const passkeyUnsupportedReasons = {
  webAuthnApiUnavailable: "unsupported:webAuthnApiUnavailable",
  capabilityApisUnavailable: "unsupported:capabilityApisUnavailable",
  capabilityProbeFailed: "unsupported:capabilityProbeFailed",
  platformAuthenticatorUnavailable: "unsupported:platformAuthenticatorUnavailable",
} as const;

export type PasskeyUnsupportedReason =
  (typeof passkeyUnsupportedReasons)[keyof typeof passkeyUnsupportedReasons];

export type PasskeySupportResult =
  | { supported: true }
  | { supported: false; reason: PasskeyUnsupportedReason };
 ...
 * The browser cannot distinguish an unconfigured authenticator from one disabled
 * by device policy, so both map to `platformAuthenticatorUnavailable`.
 */
export const checkPlatformPasskeySupport = async (): Promise<PasskeySupportResult> => {
  if (!window.PublicKeyCredential) {
    return { supported: false, reason: passkeyUnsupportedReasons.webAuthnApiUnavailable };
  }
  ...
  if (typeof publicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable === "function") {
    probes.push(
      Promise.resolve().then(async () =>
        Boolean(await publicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable?.()),
      ),
    );
  }
  ...
  if (probes.length === 0) {
    return { supported: false, reason: passkeyUnsupportedReasons.capabilityApisUnavailable };
  }
  ...
  if (results.every(result => result.status === "rejected")) {
    return { supported: false, reason: passkeyUnsupportedReasons.capabilityProbeFailed };
  }
  return {
    supported: false,
    reason: passkeyUnsupportedReasons.platformAuthenticatorUnavailable,
  };
};
...
const usePasskeyRegistration = (): {
  ...
  const attemptPreauthRegistration = useCallback(
    async (
      ...
      const support = await checkPlatformPasskeySupport();
      ...
      try {
        ...
        const makeCredentialOptions =
          fido2Util.convertPublicKeyParametersToStandardBase64(creationOptions);
