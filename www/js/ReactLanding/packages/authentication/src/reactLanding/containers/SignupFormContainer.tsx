...
import { dataStores, localStorageService } from "@rbx/core-scripts/legacy/core-roblox-utilities";
...
import { isUnderThresholdAgeForCountry } from "@rbx/authentication-common/utils/complianceUtils";
import {
  isValidBirthday,
...
import { isValidEmail } from "@rbx/authentication-common/utils/formUtils";
import { fetchReturnUrl } from "@rbx/authentication-common/utils/requestUtils";
import { parseCaptchaData } from "@rbx/authentication-common/utils/errorParsingUtils";
...
import logoutService from "@rbx/authentication-common/services/logoutService";
...
import { FeatureSignupComplianceModal } from "@rbx/authentication-common/constants/translationConstants";
import {
  logoutMessage,
  invalidParentLinkMessage,
} from "@rbx/authentication-common/constants/commonConstants";
...
} from "@rbx/authentication-common/types/signupTypes";
import {
  TCaptchaInputParams,
  TOnCaptchaChallengeCompletedData,
  TOnCaptchaChallengeInvalidatedData,
} from "@rbx/authentication-common/types/captchaTypes";
import { IOtpSession } from "@rbx/authentication-common/types/otpTypes";
import { EmailVerifyCodeModalParams } from "../../emailVerifyCodeModal/interface";

// hooks
import useOtpMetadata from "@rbx/authentication-common/hooks/useOtpMetadata";
import useIntlAuthCompliance from "@rbx/authentication-common/hooks/useIntlAuthCompliance";
import useInfoModal from "@rbx/authentication-common/hooks/useInfoModal";
...
import {
  ...
  navigateToPage,
  navigateToLogin,
} from "@rbx/authentication-common/utils/browserUtils";
...
import useSignupCheckboxExperiment from "../hooks/useSignupCheckboxExperiment";
...
import { usernameValidatorErrorMessages } from "@rbx/authentication-common/constants/validationConstants";
...
const legacySignupTelemetry = createAuthFlowTelemetry({
  flow: authFlow.signup,
  source: authSource.legacy,
  variant: authVariant.control,
});
...
export const SignupFormContainer: React.FC<SignupFormContainerProps> = ({
  ...
  const signupSilentUpgradeBrowserCheck =
    experiments.signupSilentUpgradeBrowserCheck as SilentPasskeyUpgradeVariant;
  ...
  useEffect(() => {
    const updatePasskeySupportState = async () => {
      ...
      try {
        const clientCapabilities = await (
  ...
  const [isKoreaSingupCompliancePoliciesAgreed, setIsKoreaSingupCompliancePoliciesAgreed] =
    useState(false);
  ...
  // account switch
  const [isAccountSwitchingEnabledForBrowser, isAccountSwitcherHookCompleted] =
  ...
  useEffect(() => {
    ...
    if (!isVPCSignup) {
      legacySignupTelemetry.pageMounted();
    }
  }, [isVPCSignup]);
  ...
  useEffect(() => {
    if (
      ...
      legacySignupTelemetry.formReady(!loggedInUsers.isAccountLimitReached);
    }
  }, [isGettingLoggedInUsers, isNavigating, isVPCSignup, loggedInUsers.isAccountLimitReached]);
  ...
  const browserTrackerId = getBrowserTrackerId().toString();
  ...
  function splitKeystrokeData(data: KeystrokeEvent[]): {
  ...
  const signupWithParams = async (params: TSignupParams, returnUrlValue: string) => {
    try {
      await submitSignup({
        ...
        telemetry: isVPCSignup ? undefined : legacySignupTelemetry,
      });
    } catch (error) {
  ...
  const handleSubmit = (isUserTriggered: boolean) => {
    ...
    if (isUserTriggered && !isVPCSignup) {
      legacySignupTelemetry.primaryActionClicked(authMethod.password);
      const {
    ...
    if (!isFormValid()) {
      if (isUserTriggered && !isVPCSignup) {
        legacySignupTelemetry.submissionBlocked(
          authMethod.password,
          authFailureReason.clientValidation,
        );
      }
      focusFirstFieldWithError();
      return;
