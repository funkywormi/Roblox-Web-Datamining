import { getAbsoluteUrl, isAbsolute } from "@rbx/core-scripts/endpoints";
...
export const getSafeReturnUrlFromQueryString = (): string => {
  ...
  try {
    return getAbsoluteUrl(sanitizeUrl(returnUrl));
  } catch (e) {
...
export const getSafeLoginRedirectReturnUrl = (): string => {
  ...
  if (allowedLoginRedirectReturnUrls.some(allowedUrl => getAbsoluteUrl(allowedUrl) === returnUrl)) {
    return returnUrl;
  }
