...
const usernameStartsOrEndsWithUnderscore = (username: string): boolean => {
  ...
  if (trimmedUsername.startsWith("_") || trimmedUsername[l - 1] === "_") {
    return true;
  }
  return false;
};
...
const usernameRegexInvalid = (username: string): boolean => {
  let regexFail = username.includes(" ");
  const re = /^[a-zA-Z0-9_]*$/;
...
export const isUsernameLengthValid = (username: string): boolean =>
  !usernameTooShort(username) && !usernameTooLong(username);

export const areUsernameCharactersValid = (username: string): boolean =>
  !containsHtml(username) && !usernameRegexInvalid(username);

export const isUsernameUnderscorePlacementValid = (username: string): boolean =>
...
const getInvalidUsernameMessage = (username: string): string => {
  ...
  if (!isUsernameUnderscorePlacementValid(username)) {
