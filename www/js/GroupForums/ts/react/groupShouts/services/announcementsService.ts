...
const mapResponseToModel = (raw: AnnouncementResponse): AnnouncementModel => {
  ...
  if (raw.message.contentToDisplay?.plainText != null) {
    content = {
      plainText: raw.message.contentToDisplay.plainText,
      slate: parseSlate(raw.message.contentToDisplay.slate)
    };
  } else {
...
type GetAnnouncementsByIdsOptions = {
  announcementIds: string[];
  cursor?: never;
  limit?: never;
};

type GetAnnouncementsPageOptions = {
  announcementIds?: never;
  cursor?: string;
  limit?: number;
};

type GetAnnouncementsOptions = GetAnnouncementsByIdsOptions | GetAnnouncementsPageOptions;

const getAnnouncements = async (
  groupId: number,
  { announcementIds, cursor, limit }: GetAnnouncementsOptions = {}
): Promise<AnnouncementsPageResponse<AnnouncementModel>> => {
  const urlConfig = {
  ...
  if (announcementIds?.length) {
    ...
    if (limit) usp.append('limit', limit.toString());
    params = usp;
  }
...
const getAnnouncementById = async (
  ...
  const page = await getAnnouncements(groupId, { announcementIds: [announcementId] });
