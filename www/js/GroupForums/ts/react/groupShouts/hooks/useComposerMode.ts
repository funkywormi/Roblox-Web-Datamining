...
/**
 * Resolves the composer's mode from a published announcement, router state, or the drafts query.
 *
 * - A supplied or routed published announcement takes precedence, and drafts are not queried.
...
export const useComposerMode = (
  groupId: number,
  initialPublishedAnnouncement?: AnnouncementModel
): UseComposerModeResult => {
  const location = useLocation<LocationState>();
  const publishedAnnouncement = initialPublishedAnnouncement ?? location.state?.announcement;
