...
export type AbuseReportDialogProps = AbuseReportDialogBaseProps & AbuseReportDialogRequestInput;
...
export type AbuseReportDialogWithConfigProps = AbuseReportDialogWithConfigBaseProps &
  AnalyticsTargetInput;
 ...
 *
 */
const AbuseReportDialog = (props: AbuseReportDialogProps): React.ReactElement | null => {
  const { abuseVector, open, onClose, onCloseAutoFocus, loaders, clientActions } = props;
  const requestAttributes = getRequestAttributes(props);
  const serializedAttributes = serializeAbuseReportAttributes(requestAttributes);
  ...
  useEffect(() => {
    ...
    if (open) {
      ...
      setConfig(null);
      setHasError(false);
      retryNTimes(3, () => fetchCachedConfig(abuseVector, locale, serializedAttributes))
        .then(data => {
          if (isActive) {
            setConfig(data);
            setHasError(false);
...
export const AbuseReportDialogWithConfig = (
  ...
  const { config, abuseVector, open, onClose, onCloseAutoFocus, loaders, clientActions } = props;
  const analyticsTargetId = getAnalyticsTargetId(props);
