...
import Constants from '../../creditBalancePayment/constants/Constants';
import { COUNTER_METRICS, TRANSLATION_KEYS, SYSTEM_FEEDBACK_CONFIG } from '../constants/constants';
import {
...
import { TProcessPayment, TNextPurchasableMetadata } from '../constants/typeDefinitions';
...
const handleCreditConversionToProductSwitch = async (
  ...
  try {
    fireEvent(COUNTER_METRICS.CREDIT_CONVERSION_SWITCHED_TO_PRODUCT_PURCHASE);

    await getNextPurchasableAndProcessFn(updatedAddress);

    return true;
  } catch (metadataError) {
    fireEvent(COUNTER_METRICS.CREDIT_CONVERSION_PRODUCT_SWITCH_FAILED);
    return false;
  }
};
...
> = ({ children, systemFeedbackService, translate, onSuccess }) => {
  ...
  const apiCaller = useMemo(() => createApiCaller(systemFeedbackService, translate), [
  ...
  const handlePaymentSuccess = useCallback(
    async (isSuccessful: boolean, responseMessage: string) => {
      if (isSuccessful) {
        ...
        setIsPurchaseModalOpen(false);
        setIsConvertModalOpen(false);
        fireEvent(`${COUNTER_METRICS.PROCESS_PAYMENT_NOT_SUCCESSFUL_PREFIX}${responseMessage}`);
      }

      fireEvent(`${COUNTER_METRICS.PROCESS_PAYMENT_RESPONSE_MESSAGE_PREFIX}${responseMessage}`);
    },
    [
      getNextPurchasable,
      numberOfPurchase,
