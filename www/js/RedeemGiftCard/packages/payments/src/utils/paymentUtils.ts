...
export interface PaymentProcessingConfig<T extends PaymentResult = PaymentResult> {
  failedStatusPrefix: string;
  unsuccessfulPrefix?: string;
  onStatusError: (status: number) => void;
  onSuccess: (result: T, isSuccessful: boolean) => void | Promise<void>;
 ...
 */
export const processPaymentWithCommonHandling = async <T extends PaymentResult>(
  paymentCall: () => Promise<T>,
  config: PaymentProcessingConfig<T>,
): Promise<void> => {
  try {
