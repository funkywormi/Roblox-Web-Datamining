...
const withApiMetrics = createWithApiMetrics(publishMetric);
...
const scanGiftCardConfig = () => ({
  withCredentials: true,
  url: `${EnvironmentUrls.apiGatewayUrl}/payments-gateway/v1/gift-card/scan`,
});

export type ScanGiftCardResponse = {
  scannedCode: string | null;
};

export const scanGiftCard = async (
  imageBase64: string,
): Promise<ScanGiftCardResponse | undefined> => {
  const { url, ...config } = scanGiftCardConfig();
  return withApiMetrics<ScanGiftCardResponse>({
    method: "POST",
    url,
    config,
    data: { imageBase64 },
    eventCounterProps: { call: "ScanGiftCard" },
  });
};

export const sendGetNextPurchasableMetadataStatusEvent = (success: boolean): void => {
  paymentFlowAnalyticsService.sendUserPurchaseStatusEvent(
    paymentFlowAnalyticsService.ENUM_TRIGGERING_CONTEXT.WEB_REDEEM_PAGE,
    success
