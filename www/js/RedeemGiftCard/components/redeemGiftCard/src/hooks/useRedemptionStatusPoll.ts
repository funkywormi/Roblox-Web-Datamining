...
import { trackCounter } from "@rbx/payments/creditCheckout";
import {
  pollRedemptionStatus,
  REDEMPTION_POLL_OUTCOMES,
  REDEMPTION_STATUS_FAILURE_REASONS,
} from "@rbx/payments/gift-card";
import type { RedemptionStatusResponse, RedemptionStatusFailure } from "@rbx/payments/gift-card";
...
const useRedemptionStatusPoll = ({
  ...
  const startPoll = useCallback((workflowId: string) => {
    ...
    const run = async () => {
      try {
        const result = await pollRedemptionStatus({ workflowId, isCancelled: isStale });
        if (isStale()) {
          return;
        }
  ...
  useEffect(
    () => () => {
      ...
      if (inFlightRef.current) {
        trackCounter("GiftCard_RedeemPollUnmounted");
      }
    },
    [],
  );
